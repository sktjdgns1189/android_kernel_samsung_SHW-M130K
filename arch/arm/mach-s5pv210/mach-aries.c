...
static struct max8998_platform_data max8998_pdata = {
	...
	.buck1_voltage_set	= { 1275000, 1225000, 1075000, 975000 },
...
static int s5ka3dfx_power_on(void)
{
	...
	if(HWREV <= 5)	//Arian-K : seperate vga camera power
		gpio_free(GPIO_CAM_IO_EN);
...
void config_sleep_gpio(void)
{
	...
	s3c_gpio_setpull(S5PV210_GPH2(1), S3C_GPIO_PULL_NONE); //FOCUS key	
	s3c_gpio_setpull(S5PV210_GPH2(2), S3C_GPIO_PULL_NONE); //CAMERA key
