...
/* S1_KOR [ Reduce fw load wait time ] */
enum cam_vendor {
	CAM_VENDOR_NONE = 0,	// None
	CAM_VENDOR_SE = 1,		//Samsung Electric
	CAM_VENDOR_TE = 2,		//Techwin 
	CAM_VENDOR_OP = 3		//Samsung Opt
};
static enum cam_vendor CamVendorType = CAM_VENDOR_NONE;
		...
		unsigned char *r_data, unsigned int r_len)
{
	...
	retry_count = 2;		// change retry count 1->2
	while (retry_count--) {
		ret = i2c_transfer(client->adapter, &msg, 1);
		if (ret == 1)
			break;
	...
	if(ret < 0)
	{
		dev_err(&client->dev, "%s: I2C read ERROR -- FAIL \n", __func__);
		return -EIO;
	}
	...
	{
		int j;
		pr_debug("R: read_data =");
		for(j = 0; j < r_len; j++){
			pr_debug("0x%02x ", r_data[j]);
		}
		pr_debug("\n");
	}
#endif
...
static int ce147_read_fw_bin(const char *path, char *fwBin, int *fwSize)
{
	...
	//pr_debug("ce147_read_main_SW_fw_version is called...\n");
...
#if 1	//S1_KOR [ Reduce fw load wait time ]
	pr_debug("%s : vendor %d", __func__, CamVendorType);

	/* At least 700ms delay	required to load the firmware
	 * for ce147 camera ISP	*/
	// This is modified from 700ms to 800ms by STW
	if((CamVendorType == CAM_VENDOR_NONE) || (CamVendorType == CAM_VENDOR_TE))
		msleep(800);
	else
		msleep(700);
...
static int ce147_get_fw_version(struct v4l2_subdev *sd)
{
	...
	if (err < 0)
	{
       	pr_debug("%s : failed... get FW version() \n", __func__); 	
		return err;
	}
	...
	/* S1_KOR [ Reduce fw load wait time ] */
	if(state->fw.major == 0x05)
		CamVendorType = CAM_VENDOR_SE;
	else if(state->fw.major == 0x0F)
		CamVendorType = CAM_VENDOR_TE;
	else if(state->fw.major == 0x31)
		CamVendorType = CAM_VENDOR_OP;
	else
		pr_debug("There is no Vendor Type %d", state->fw.major);
...
static int ce147_get_dateinfo(struct v4l2_subdev *sd)
{
	...
	if (err < 0)
	{
       	pr_debug("%s : failed... get DATE information \n", __func__); 	
		return err;
	}
...
static int ce147_get_sensor_version(struct v4l2_subdev *sd)
{
	...
	if (err < 0)
	{
       	pr_debug("%s : failed... get SENSOR version \n", __func__); 	
		return err;
	}
					...
					unsigned char *value)
{
	...
	if ((ce147_buf_get_af_status[0]	& 0x01)	!= 0x00)
	{
		dev_err(&client->dev, "%s: failed: TIME OUT - status %d\n", __func__, ce147_buf_get_af_status[0]);		//debug
		return -EBUSY;
	}
	else
...
static int ce147_set_iso(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int err;
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ce147_state *state = to_state(sd); //Nightshot/BeachSnow/Firework delay of LSI Sensor
	...
	state->iso = ctrl->value; //Nightshot/BeachSnow/Firework delay of LSI Sensor
...
static int ce147_init(struct v4l2_subdev *sd, u32 val)
{
	...
	printk(KERN_ERR "==== MAIN SW FW INFO ====\n");
	printk(KERN_ERR "FW M:%d m:%d |PRM M:%d m:%d \n", MAIN_SW_FW[0], MAIN_SW_FW[1], MAIN_SW_FW[2], MAIN_SW_FW[3]);
	printk(KERN_ERR "y. m. d = %d.%d.%d \n", MAIN_SW_DATE_INFO[0], MAIN_SW_DATE_INFO[1], MAIN_SW_DATE_INFO[2]);

	printk(KERN_ERR "==== SENSOR FW INFO ====\n");
	printk(KERN_ERR "FW  Version: %d.%d\n", state->fw.major, state->fw.minor);
	printk(KERN_ERR "PRM Version: %d.%d\n", state->prm.major, state->prm.minor);
	printk(KERN_ERR "Date(y.m.d): %d.%d.%d\n", state->dateinfo.year, state->dateinfo.month, state->dateinfo.date);	
	printk(KERN_ERR "Sensor version: %d\n", state->sensor_version);	
