				  ...
				  const char *buf, size_t size)
{
	...
	bool new_value;
#if 0
	// code blocked by null pointer kernel panic
	if (sysfs_streq(buf, "1"))
		new_value = true;
	else if (sysfs_streq(buf, "0"))
		new_value = false;
    ...
    if(gp2a != NULL) {
		...
		gp2a_dbgmsg("gp2a is null or blocked\n");
	}
	//mutex_unlock(&gp2a->power_lock);
#endif
	return size;
...
static DEVICE_ATTR(lightsensor_file_cmd, 0664, lightsensor_file_cmd_show, lightsensor_file_cmd_store);
...
static ssize_t proxsensor_file_state_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	struct gp2a_data *ip = dev_get_drvdata(dev);
	int val = gpio_get_value(ip->pdata->p_out);
...
static DEVICE_ATTR(call_proximity_onoff,0644, call_proximity_onoff_control_show, call_proximity_onoff_control_store);
static DEVICE_ATTR(call_proximity_onoff_earjack,0644, call_proximity_onoff_contro_earjack_show, call_proximity_onoff_control_earjack_store);
static DEVICE_ATTR(proxsensor_factory_mode,0644, proxsensor_factory_mode_show, proxsensor_factory_mode_store);
#endif
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (device_create_file(gp2a->switch_cmd_dev, &dev_attr_lightsensor_file_cmd) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_lightsensor_file_cmd.attr.name);

	if (device_create_file(gp2a->switch_cmd_dev, &dev_attr_lightsensor_file_state) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_lightsensor_file_state.attr.name);

	gp2a->proxsensor_class = class_create(THIS_MODULE, "proxsensor");
	if (IS_ERR(gp2a->proxsensor_class))
		pr_err("Failed to create class(proxsensor)!\n");	
	
	gp2a->switch_cmd_dev = device_create(gp2a->proxsensor_class,
		NULL, 0, NULL, "switch_cmd");
	if (IS_ERR(gp2a->switch_cmd_dev))
		pr_err("Failed to create device(switch_cmd_dev)!\n");	
	
	if (device_create_file(gp2a->switch_cmd_dev, &dev_attr_proxsensor_file_state) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_proxsensor_file_state.attr.name);

#if 1

	if (device_create_file(gp2a->switch_cmd_dev, &dev_attr_gp2asensor_i2c_state) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_gp2asensor_i2c_state.attr.name);


	if (device_create_file(gp2a->switch_cmd_dev, &dev_attr_proxsensor_factory_mode) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_proxsensor_factory_mode.attr.name);
		
	if (device_create_file(gp2a->switch_cmd_dev, &dev_attr_call_proximity_onoff) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_call_proximity_onoff.attr.name);
	
	if (device_create_file(gp2a->switch_cmd_dev, &dev_attr_call_proximity_onoff_earjack) < 0)
