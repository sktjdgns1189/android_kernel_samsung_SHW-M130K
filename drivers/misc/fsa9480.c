...
static void fsa9480_reset(struct fsa9480_usbsw *usbsw)
{
	struct i2c_client *client = usbsw->client;
	int ret;

	/* reset */
	ret = i2c_smbus_write_word_data(client, FSA9480_REG_MANUAL_OVERRIDES1, 0x01);
	if (ret < 0)
		dev_err(&client->dev, "%s: err %d\n", __func__, ret);
}
...
static irqreturn_t fsa9480_irq_thread(int irq, void *data)
{
	...
	if (intr < 0) {
		dev_err(&client->dev, "%s: err %d\n", __func__, intr);
		intr = i2c_smbus_read_byte_data(client, FSA9480_REG_INT1);
		if (intr < 0)
			dev_err(&client->dev, "%s: err at read %d\n", __func__, intr);
		intr = i2c_smbus_read_byte_data(client, FSA9480_REG_INT2);
		if (intr < 0)
			dev_err(&client->dev, "%s: err at read %d\n", __func__, intr);
...
static int fsa9480_resume(struct i2c_client *client)
{
	...
	if(ftm_sleep && ftm_sleep_exit)
	{
		...
		printk("wakeup from ftm sleep, port reconnect..\n");
	}
#endif
	/* device detection */
	fsa9480_detect_dev(usbsw);
