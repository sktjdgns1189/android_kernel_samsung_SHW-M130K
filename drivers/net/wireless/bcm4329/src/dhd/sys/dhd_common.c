...
/*
{
	uint8 *macid = (uint8 *)&dhd->mac;
	printk("%s: MACID %02X:%02X:%02X:%02X:%02X:%02X\n", __FUNCTION__, 
		macid[0],macid[1],macid[2], macid[3],macid[4], macid[5]);
}
*/
	bcm_mkiovar("cur_etheraddr", (char*)&dhd->mac, ETHER_ADDR_LEN, iovbuf, sizeof(iovbuf));
	if (dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, \
			iovbuf, sizeof(iovbuf)) < 0)
			DHD_ERROR(("%s: Set MACID failed\n", __FUNCTION__));
